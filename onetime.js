...
function fetchData(childSnapshot, snapshot) {
    ...
    row.insertCell(7).innerHTML = childSnapshot.child('status').val();
    if (row.cells[7].innerHTML !== 'Resumed') {
        row.style.backgroundColor = 'red';
        row.style.color = 'white';
    }
    row.insertCell(8).innerHTML = convertTime(childSnapshot.child('timeStamp').val());
    row.insertCell(9).innerHTML = childSnapshot.child('address').val();
    row.insertCell(10).innerHTML = childSnapshot.child('total').val();
}
